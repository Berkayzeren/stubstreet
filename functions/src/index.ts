...
app.post('/v1/paytr/initialize', 
  authMiddleware, // Authentication gerekli
  ...
  try {
    ...
    const paytrToken = crypto.createHmac('sha256', PAYTR_MERCHANT_KEY as string).update(paytrTokenRaw).digest('base64');

    // Türkçe karakterleri temizle
    const cleanTurkishChars = (text: string) => {
      return text
        .replace(/ğ/g, 'g')
        .replace(/Ğ/g, 'G')
        .replace(/ü/g, 'u')
    ...
    const sanitizeBasket = (basketInput: any[]): any[] => {
      if (!Array.isArray(basketInput)) return [];
      return basketInput.map((item: any) => {
        const arr = Array.isArray(item) ? item : [];
        const name = cleanTurkishChars(String(arr[0] ?? 'Item')).replace(/[^\x20-\x7E]/g, '');
        const priceRaw = String(arr[1] ?? '0');
        ...
        const qty = String(arr[2] ?? '1').replace(/[^0-9]/g, '') || '1';
    ...
    const noInstallmentCalculated = String(
      noInstallment !== undefined
    ...
    console.log('🔍 PayTR Request Data:', {
      ...paytrData,
      paytr_token: '[HIDDEN]',
      user_basket_json: sanitizedBasket,
